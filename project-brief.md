...
Filenames are semantic and map directly to the component specs below: `hero-{page}.jpg`, `home-*.jpg`, `estate-*.jpg`, `house-main-*.jpg`, `house-cottage-*.jpg`, `experience-*.jpg`, `exp-*.jpg` (home-page grid), `dining-*.jpg`, `journal-*.jpg`, `gallery/gallery-NN.jpg` (+ `gallery/thumbs/`, `gallery/full/` variants), `logo/pax-logo-transparent-500.png` (header badge, enquiry mini header, footer watermark — the original shield logo; the owner rejected a redrawn horse-only badge, so keep the real brand art). `logo/pax-badge.svg` (horse + circular text) exists on disk but is **unused** — do not wire it in. `quote-band.jpg` is currently unused (the photo quote band was replaced by the framed testimonial).
...
- Center: the **logo badge** — an ivory **rounded-rect plate that hugs the logo's shape** (`position:absolute`, centered, `top:16px`; 96×114px, `border-radius:22px`; 70×84px radius 16px on mobile), holding the original shield logo (`images/logo/pax-logo-transparent-500.png`, 74px wide / 54px mobile). Not a circle — the owner replaced the circle with this plate so the container echoes the logo's own rounded-rectangle silhouette. The owner also explicitly wants the real brand logo here — a redrawn horse-only mark was rejected as off-brand. Deliberately overlaps down onto the hero image below. **The plate must blend seamlessly with the bar** (same ivory, no visible edge or shadow halo inside the header): the shadow lives on a `::before` (`border-radius:inherit`) clipped with `clip-path: inset(60px -48px -48px -48px)` — bar bottom (76px) minus badge top (16px) — so it renders only on the part hanging below the bar (62px top inset at the mobile sizes: 66px bar, plate top 4px — the **visible logo art's** top sits at 13px, level with the Reserve button's top, per owner; the plate's padding hides above). Don't "fix" the missing top shadow; it's the point. Keep it absolutely positioned: placing it in grid flow inflates the row and pushes the links below the bar.
