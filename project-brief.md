...
Filenames are semantic and map directly to the component specs below: `hero-{page}.jpg`, `home-*.jpg`, `estate-*.jpg`, `house-main-*.jpg`, `house-cottage-*.jpg`, `experience-*.jpg`, `exp-*.jpg` (home-page grid), `dining-*.jpg`, `journal-*.jpg`, `gallery/gallery-NN.jpg` (+ `gallery/thumbs/`, `gallery/full/` variants), `logo/pax-badge.svg` (header badge, enquiry mini header, footer watermark — the horse encircled by "PAX / RANCH HOUSE" text, no shield). The shield versions (`logo/pax-logo*.png`, `logo/pax-logo.svg`) remain for print/favicon use. `quote-band.jpg` is currently unused (the photo quote band was replaced by the framed testimonial).
...
- Center: the **logo badge** — a 104px ivory circle (`position:absolute`, centered, `top:16px`) with the circular badge art (`images/logo/pax-badge.svg`: the galloping horse with "PAX" arcing over the top and "RANCH HOUSE" under the bottom — no shield rectangle; the round art fills the round badge like the reference). Deliberately overlaps down onto the hero image below. **The circle must blend seamlessly with the bar** (same ivory, no visible edge or shadow halo inside the header): the shadow lives on a `::before` clipped with `clip-path: inset(60px -48px -48px -48px)` — bar bottom (76px) minus badge top (16px) — so it renders only on the arc hanging below the bar (46px top inset at the mobile sizes: 66px bar, badge top 20px, so ~60% of the badge nests inside the bar on all viewports). Don't "fix" the missing top shadow; it's the point. Keep it absolutely positioned: placing it in grid flow inflates the row and pushes the links below the bar.
